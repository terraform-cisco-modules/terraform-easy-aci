...
variable "date_and_time" {
  ...
  description = <<-EOT
    Key - Name for the Date and Time Pod Policy
    * annotation: (optional) — An annotation will mark an Object in the GUI with a small blue circle, signifying that it has been modified by  an external source/tool.  Like Nexus Dashboard Orchestrator or in this instance Terraform.
    * administrative_state: (optional) — The NTP protocol can be enabled or disabled. If enabled, the system uses the NTP protocol. The NTP protocol can be either of the following:
      - enabled: (default) — NTP protocol is enabled.
      - disabled — NTP protocol is disabled.
    * authentication_keys: (optional) — List of NTP Authentication keys; when performing NTP Authentication.
      - authentication_type: (optional) — The authentication key type can be:
        * md5 — Use HMAC MD5 algorithm for authentication
        * sha1: (default) — Use HMAC SHA1 algorithm for authentication
      - key_id: (required) — The user-configured NTP client authentication key name. The NTP client authentication key is for authenticating time synchronization. This name can be between 1 and 65335. Note that you cannot change this name after the object has been saved.
      - trusted: (optional) — The trust status of the authentication key. This indicates if the NTP authentication key is trusted. The trust states of the NTP authentication key follows:
        * false — The authentication key is not trusted.
        * true: (default) — The authentication key is trusted.
    * description: (optional) — Description to add to the Object.  The description can be up to 128 characters.
    * display_format: (optional) — The date/time display format selector. The display format can be UTC or local. The date/time display format can be either of the following:
      - utc — Date/time is displayed in UTC.
      - local: (default) — Date/time is displayed in local time.
    * master_mode: (optional) — Enables the designated NTP server to provide undisciplined local clock time to downstream clients with a configured stratum number. For example, a leaf switch that is acting as the NTP server can provide undisciplined local clock time to leaf switches acting as clients.  The options are:
      - enabled — The designated NTP server provides undisciplined local clock time.
      - disabled: (default) — The designated NTP server does not provide undisciplined local clock time.
      Notes:
    ...
    * ntp_servers: (optional) — List of NTP Servers.
      ...
      - minimum_polling_interval: (default: 4) — The minimum NTP default polling value. This default is set in terms of seconds. The minimum interval range is from 4 to 16.
      - preferred: (optional) — Indicates if the NTP server is preferred. Multiple preferred servers can be configured. The NTP server preference states include the following:
        * false: (default) — The NTP server is not preferred.
        * true — The NTP server is preferred.
    * offset_state: (default: ) — The display of the offset can be enabled or disabled. This enables you to view the difference between the local time and the reference time. 
      - enabled: (default)
      - disabled
    * server_state: (optional) — Enables switches to act as NTP servers to provide NTP time information to downstream clients. The options are:
      - enabled — Switch, while being acting as a client, serves the time as server to other clients.
      - disabled: (default) — Switch acts as a client to an NTP server
    ...
    * stratum_value: (default: 8) — Specifies the stratum level from which NTP clients get their time synchronized. The range is from 1 to 15.  The default master mode stratum number is 8.
    * time_zone: (optional) — The time zone selection. This enables you to select a time zone for your fabric.  Valid Timezones:
      - Africa/Abidjan
      - Africa/Accra
      - Africa/Addis_Ababa
      - Africa/Algiers
